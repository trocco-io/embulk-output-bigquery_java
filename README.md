...
`test_host` is an internal, undocumented-for-users config option used by this plugin's own test suite (`TestBigqueryJavaOutputPlugin`) to redirect all BigQuery API calls to a local `MockWebServer` instead of the real BigQuery API, and to bypass credential setup entirely (`NoCredentials`). It is not intended for use outside of tests.
